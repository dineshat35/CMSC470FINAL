...
def add_buzzer_params(parser):
    ...
    parser.add_argument('--features', nargs='+', help='Features to feed into Buzzer', type=str,  default=['Length', 'Difficulty'])    
...
def load_buzzer(flags, load=False):
    ...
    for ff in flags.features:
        ...
        if ff == "Frequency":
            ...
            features_added.add(ff)
        if ff == "Palindrome":
            from features import PalindromeCountFeature
            feature = PalindromeCountFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff) 
        if ff == "Plural":
            from features import SingularPluralFeature
            feature = SingularPluralFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff) 
        if ff == "Difficulty":
            from features import DifficultyLevelFeature
            feature = DifficultyLevelFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff) 
